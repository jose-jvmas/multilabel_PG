...
class MRSP3:
    ...
    def getMostDistantPrototypes(self, in_set):
        ...
        for single_duple in duples:
            curr_dist = distance.euclidean(
                in_set[single_duple[0]], in_set[single_duple[1]]
            )
            if curr_dist > max_dist:
    ...
    def divideBIntoSubsets(self, B, p1, p2):
        B1_indexes = np.where(
            np.array(
                [
                    distance.euclidean(B[u], B[p1]) <= distance.euclidean(B[u], B[p2])
    ...
    def checkClusterCommonLabel(self, in_elements):
        ...
        common_label_vec = [
            len(np.nonzero(in_elements[:, it] == 1)[0]) == len(in_elements)
            for it in range(in_elements.shape[1])
        ]
        ...
        while len(Q) > 0:
            ...
            for single_partition in [B1, B2]:
                ...
                if len(single_partition) > 0:
                    if (
                        self.checkClusterCommonLabel(single_partition[1])
                        or single_partition[1].shape[0] == 1
                    ):
