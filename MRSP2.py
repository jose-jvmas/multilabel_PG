...
class MRSP2:
        ...
        if len(sorted_in_list) > 1:
            ...
            for it_src_element in range(len(sorted_in_list)):
                for it_dst_element in range(it_src_element, len(sorted_in_list)):
                    curr_dist = self.distances_dict[sorted_in_list[it_src_element]][
                        sorted_in_list[it_dst_element]
                    ]
                    if curr_dist > max_dist:
                        most_distant_duple = (
                            sorted_in_list[it_src_element],
                            sorted_in_list[it_dst_element],
                        )
                        max_dist = curr_dist
        else:
    ...
    def divideBIntoSubsets(self, B, p1, p2):
        B1_indexes = [
            ...
            for idx in np.where(
                np.array(
                    [
                        self.distances_dict[min(u, p1)][max(u, p1)]
                        <= self.distances_dict[min(u, p2)][max(u, p2)]
                        for u in B
                    ]
                )
                == True
    ...
    def computePairwiseDistances(self):
        ...
        for it_row in range(self.X.shape[0]):
            ...
            for it_col in range(it_row, self.X.shape[0]):
                current_row_dict[it_col] = distance.euclidean(
                    self.X[it_row], self.X[it_col]
                )
    ...
    def reduceSet(self, X, y, params):
        ...
        for _ in range(n_out - 1):
            ...
            prototypeIndexesQchosen_dict[i] = (
                self.getMostDistantPrototypes(B1) if len(B1) > 0 else ()
            )
            prototypeIndexesQchosen_dict[bc] = (
                self.getMostDistantPrototypes(B2) if len(B2) > 0 else ()
            )
...
if __name__ == "__main__":
    ...
    X_red, y_red = MRSP2().reduceSet(
        X_train.toarray().copy(), y_train.toarray().copy(), 70
    )
    ...
    print("Results:")
    print(
        "\t - Hamming Loss: {:.3f} - Size: {:.1f}%".format(
            hamming_loss(y_test, y_pred_ori), 100 * X_train.shape[0] / X_train.shape[0]
        )
    )
    print(
        "\t - Hamming Loss: {:.3f} - Size {:.1f}%".format(
            hamming_loss(y_test, y_pred_red), 100 * X_red.shape[0] / X_train.shape[0]
        )
    )
    print("Done!")
